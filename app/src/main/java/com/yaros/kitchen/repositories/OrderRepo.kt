...
class OrderRepo(val db: RoomDb,val rx: RxSchedulers) {
    ...
    fun deleteOrderById2(id: String) {
        db.KitchenOrderDAO().deleteItemById(id)
    }
    ...
    fun checkOrder(order: String): Boolean? =
        db.KitchenOrderDAO().checkItem(order)/*.compose(rx.applyObservable())?.blockingFirst()*/
...
//        return LivePagedListBuilder(RoomFactoryWrapper( db.KitchenOrderDAO().getOrderByPrinter(orderId)), pagedListConfig).build()

        return LivePagedListBuilder<Int, KitchenOrderModel>(
            db.KitchenOrderDAO().getOrderByPrinter(orderId), pagedListConfig
        ).build()
    }
